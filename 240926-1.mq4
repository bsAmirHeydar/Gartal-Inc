...
void detection_candle() {
    ...
    buy_entry_condition  = detection240919_ssl_hybrid_SSL2(1) && detection240919_cm_william_fix_vix(1) && major_technical_management(1) && detection240919_minor_reverse_ind(1) && detection240921_MA_cross(1) && detection240923_blackFlag_FTS(1) && detection240923_MACD_SMA(1) && check_candle_color_check(1) && check_last_candle_color_check(1);
    sell_entry_condition = detection240919_ssl_hybrid_SSL2(-1) && detection240919_cm_william_fix_vix(-1) && major_technical_management(-1) && detection240919_minor_reverse_ind(-1) && detection240921_MA_cross(-1) && detection240923_blackFlag_FTS(-1) && detection240923_MACD_SMA(-1) && check_candle_color_check(-1) && check_last_candle_color_check(-1);
    buy_entry_price      = Bid;
...
void OnTick() {
    ...
    if(
        (currentHour < end_hour || (currentHour == end_hour && currentMinute < end_minute)) && (currentHour > start_hour || (currentHour == start_hour && currentMinute > start_minute))) {
        detection();
    }
}
//+------------------------------------------------------------------+
